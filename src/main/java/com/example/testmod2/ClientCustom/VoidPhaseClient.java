...
public class VoidPhaseClient {
    ...
    private static final Identifier VOID_PHASE_IDLE_EFFECT =
            Identifier.fromNamespaceAndPath(TestMod2.MODID, "void_phase");
    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        boolean localClientFlag = Boolean.TRUE.equals(SpatialSwordEffect.IN_VOID);
        boolean inVoid = attachmentInVoid || localClientFlag || usingSpatialSword;      //根据条件判断是否在虚空
        ...
        if (inVoid != lastResolvedInVoid) {                         //如果虚空状态发生改变 发送一条各组件状态的日志
            LOGGER.info("[VoidPhase] resolved inVoid={} (attachment={}, clientFlag={}, usingSword={})",
                    inVoid, attachmentInVoid, localClientFlag, usingSpatialSword);
        ...
        if (inVoid) {
            ...
            stopLoopSound(mc);                                  //如果不在虚空内 就停止播放音频
